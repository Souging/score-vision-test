...
async def process_soccer_video(
    ...
    try:
        ...
        async for frame_number, frame in video_processor.stream_frames(video_path):
            frame_batch.append((frame_number, frame))
            
            if len(frame_batch) >= batch_size:
                
                batch_numbers = [item[0] for item in frame_batch]
                batch_frames = [item[1] for item in frame_batch]
                
                
                pitch_results = pitch_model(batch_frames, verbose=False)
                player_results = player_model(batch_frames, imgsz=1280, verbose=False)
                
                for i, (frame_number, frame) in enumerate(frame_batch):
